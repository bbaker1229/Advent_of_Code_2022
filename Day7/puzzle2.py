...
# Find the current unused space on the drive
unused_space = 70000000 - root.size
# Find the size of the space required for the update
required_for_update = 30000000 - unused_space

# Find the size of the directory to be deleted
minimum = 70000000  # Set a variable to a high value
for _, size in all_dirs.items():  # Loop through all directories
    if size >= required_for_update:  # If the directory size is high enough, continue
        # Find the amount of additional space after delete and install
        temp = size - required_for_update  
        # if this amount is smaller than the previously found value, keep it.
        minimum = min(minimum, temp)  
size_of_dir = minimum + required_for_update  # Back calc size of dir.

# Print the output
print(f"The size of the directory to delete is: {size_of_dir}.")
